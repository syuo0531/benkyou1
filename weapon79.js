...
const SPECIAL={
  wep_hammer:'weapon83-hammer.svg?v=84',
  wep_bow:'weapon83-bow.svg?v=84',
  wep_ancient:'weapon83-ancient.svg?v=84',
  wep_abyss_reaper:'weapon83-abyss.svg?v=84',
  wep_astral_blade:'weapon83-astral.svg?v=84',
  wep_crimson_burst:'weapon83-crimson.svg?v=84'
};
...
function patch(){
  document.querySelectorAll('#gearGrid .gear-item[data-gear]').forEach(btn=>{
    ...
    const mark='v84-'+g.id; if(box.dataset.weapon81===mark)return;
  ...
  try{
    ...
    if(g&&g.slot==='weapon'&&IDS.has(g.id)&&box){const mark='v84-'+g.id;if(box.dataset.weapon81!==mark){box.innerHTML=art(g,108)+aura(g);box.dataset.weapon81=mark;}}
  }catch(_){ }
}
